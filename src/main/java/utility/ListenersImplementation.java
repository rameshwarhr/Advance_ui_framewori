...
//	ExtentFactory.getInstance().getExtent().log(Status.PASS, "Test Case: "+result.getMethod().getMethodName()+ " is Passed.");
	ExtentFactory.getInstance().getExtent().log(Status.PASS, MarkupHelper.createLabel("Test Passed :- "+result.getMethod().getMethodName()+"-:-"+result.getMethod().getRealClass(), ExtentColor.GREEN));
	ExtentFactory.getInstance().removeExtentObject();
}

public void onTestFailure(ITestResult result) {
//	ExtentFactory.getInstance().getExtent().log(Status.FAIL, "Test Case: "+result.getMethod().getMethodName()+ " is Failed.");
	ExtentFactory.getInstance().getExtent().log(Status.FAIL, MarkupHelper.createLabel("Test Failed :- "+result.getMethod().getMethodName()+"-:-"+result.getMethod().getRealClass(), ExtentColor.RED));
